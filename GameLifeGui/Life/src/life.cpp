...
//    Build size for Life GUI grid

    void buildGuiGridSize(Grid<char> grid){
    ...
    void buildGrid(ifstream &in){
        ...
        try{
            // Get number of rows
              ...
              this->buildGuiGridSize(grid);

              this->printGuiBoard(grid);
...
// ***Validate the file name entered by user***
    ...
    // Build and print the GUI Board
    //game.buildGuiGrid(in);
    //game.printGuiBoard();
